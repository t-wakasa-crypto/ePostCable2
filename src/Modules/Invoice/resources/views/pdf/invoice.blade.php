{{-- 請求書 PDF テンプレート（詳細設計 §1.3.2 / FR-15）。A4 縦・日本語表示。 --}}
<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="utf-8">
...
<body>
    ...
    <table class="meta">
        <tr><td>請求書番号</td><td>{{ $invoice->invoice_number }}</td></tr>
        <tr><td>顧客名</td><td>{{ $invoice->customer_name }} 御中</td></tr>
        <tr><td>請求日</td><td>{{ optional($invoice->issue_date)->format('Y年m月d日') }}</td></tr>
    </table>
    ...
    <table class="totals">
        <tr>
            ...
            <td class="value">{{ number_format((int) $invoice->amount) }} 円</td>
        </tr>
        <tr>
            <td class="label">消費税（{{ (int) $invoice->tax }}%）</td>
            <td class="value">{{ number_format((int) $invoice->tax_amount) }} 円</td>
        </tr>
        <tr>
            <td class="label"><strong>合計（税込）</strong></td>
            <td class="value"><strong>{{ number_format((int) $invoice->amount + (int) $invoice->tax_amount) }} 円</strong></td>
        </tr>
    </table>
</body>
</html>
