{{-- 納品書詳細（FA-01 / detailed-design §1A.5 / FR-09） --}}
<x-deliverynote::layouts.master :title="'納品書詳細: ' . $deliveryNote->delivery_number">
    ...
    {{-- 書類ヘッダーカード --}}
    <section aria-label="delivery-note-meta" class="bg-white shadow-sm rounded-lg border border-gray-200 p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-gray-800">{{ $deliveryNote->delivery_number }}</h2>
            <x-shared::status-badge :status="$deliveryNote->status" />
        </div>
        <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-sm">
            <div class="flex justify-between border-b border-gray-100 pb-2"><dt class="text-gray-500">顧客名</dt><dd class="text-gray-800">{{ $deliveryNote->customer_name }}</dd></div>
            <div class="flex justify-between border-b border-gray-100 pb-2"><dt class="text-gray-500">ステータス</dt><dd class="text-gray-800">{{ $deliveryNote->status }}</dd></div>
            <div class="flex justify-between border-b border-gray-100 pb-2"><dt class="text-gray-500">納品日</dt><dd class="text-gray-800">{{ optional($deliveryNote->delivery_date)->format('Y-m-d') }}</dd></div>
            <div class="flex justify-between border-b border-gray-100 pb-2"><dt class="text-gray-500">金額（税抜）</dt><dd class="text-gray-800">{{ number_format((int) $deliveryNote->amount) }}</dd></div>
            <div class="flex justify-between border-b border-gray-100 pb-2"><dt class="text-gray-500">消費税</dt><dd class="text-gray-800">{{ number_format((int) $deliveryNote->tax_amount) }}</dd></div>
        </dl>
    </section>

    {{-- 操作カード（詳細設計 §5.4 / §7 / FR-09 / FR-17） --}}
    <section aria-label="delivery-note-actions" class="bg-white shadow-sm rounded-lg border border-gray-200 p-6 mb-6">
        ...
        <div class="flex flex-wrap items-center gap-3">
            <form method="POST" action="{{ route('delivery-notes.resend', $deliveryNote) }}"
                onsubmit="return confirm('この納品書を再送しますか？');">
                @csrf
        ...
        @if ($isAdmin && $isFailed)
            <form method="POST" action="{{ route('delivery-notes.emails', $deliveryNote) }}" class="mt-6 border-t border-gray-100 pt-4 space-y-3">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                        ...
                        <input type="email" name="emails[]" value="{{ $deliveryNote->customer_email }}"
                    ...
                    <div>
                        ...
                        <input type="email" name="emails[]" value="{{ $deliveryNote->customer_email_2 }}"
                            class="border border-gray-300 rounded px-3 py-2 w-full focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">送付先3</label>
                        <input type="email" name="emails[]" value="{{ $deliveryNote->customer_email_3 }}"
                            class="border border-gray-300 rounded px-3 py-2 w-full focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                </div>
